  ...
  class StatementSummaryRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::StatementSummaryRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::StatementSummaryRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'statement_list')
        if (value = attributes[:'statement_list']).is_a?(Array)
          self.statement_list = value
        end
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          statement_list == o.statement_list &&
    ...
    def hash
      [card_id, currency_code, statement_list, next_start_index].hash
    end
