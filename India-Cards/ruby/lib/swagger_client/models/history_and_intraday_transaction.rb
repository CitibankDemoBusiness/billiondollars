...
module SwaggerClient
  class HistoryAndIntradayTransaction
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::HistoryAndIntradayTransaction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::HistoryAndIntradayTransaction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'activity_indicator')
        self.activity_indicator = attributes[:'activity_indicator']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          activity_indicator == o.activity_indicator &&
    ...
    def hash
      [activity_indicator, more_indicator, next_start_index].hash
    end
