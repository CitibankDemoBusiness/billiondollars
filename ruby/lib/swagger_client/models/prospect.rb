  ...
  class Prospect
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::Prospect` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::Prospect`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @contact_preference.nil?
        invalid_properties.push('invalid value for "contact_preference", contact_preference cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @contact_preference.nil?
      true
    end
    ...
    def hash
      [mother_maiden_name, name, demographics, address, email, phone, contact_preference, financial_information, education, employment_details, identification_document_details, additional_data, self_declaration, kyc_information].hash
    end
