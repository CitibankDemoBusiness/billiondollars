  ...
  class BankingProductDiscount
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::BankingProductDiscount` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::BankingProductDiscount`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      discount_type_validator = EnumAttributeValidator.new('Object', ['BALANCE', 'DEPOSITS', 'PAYMENTS', 'FEE_CAP', 'ELIGIBILITY_ONLY'])
    ...
    def discount_type=(discount_type)
      validator = EnumAttributeValidator.new('Object', ['BALANCE', 'DEPOSITS', 'PAYMENTS', 'FEE_CAP', 'ELIGIBILITY_ONLY'])
    ...
    def hash
      [description, discount_type, amount, balance_rate, transaction_rate, accrued_rate, fee_rate, additional_value, additional_info, additional_info_uri, eligibility].hash
    end
