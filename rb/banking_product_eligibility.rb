  ...
  class BankingProductEligibility
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::BankingProductEligibility` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::BankingProductEligibility`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @eligibility_type.nil?
        invalid_properties.push('invalid value for "eligibility_type", eligibility_type cannot be nil.')
      end
    ...
    def valid?
      return false if @eligibility_type.nil?
      eligibility_type_validator = EnumAttributeValidator.new('Object', ['BUSINESS', 'PENSION_RECIPIENT', 'MIN_AGE', 'MAX_AGE', 'MIN_INCOME', 'MIN_TURNOVER', 'STAFF', 'STUDENT', 'EMPLOYMENT_STATUS', 'RESIDENCY_STATUS', 'NATURAL_PERSON', 'OTHER'])
      return false unless eligibility_type_validator.valid?(@eligibility_type)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] eligibility_type Object to be assigned
    def eligibility_type=(eligibility_type)
      validator = EnumAttributeValidator.new('Object', ['BUSINESS', 'PENSION_RECIPIENT', 'MIN_AGE', 'MAX_AGE', 'MIN_INCOME', 'MIN_TURNOVER', 'STAFF', 'STUDENT', 'EMPLOYMENT_STATUS', 'RESIDENCY_STATUS', 'NATURAL_PERSON', 'OTHER'])
      unless validator.valid?(eligibility_type)
        fail ArgumentError, "invalid value for \"eligibility_type\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [eligibility_type, additional_value, additional_info, additional_info_uri].hash
    end
