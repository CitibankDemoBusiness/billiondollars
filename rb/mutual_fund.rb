...
module SwaggerClient
  class MutualFund
    ...
    attr_accessor :net_asset_value_amount
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::MutualFund` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::MutualFund`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'net_asset_value_amount')
        self.net_asset_value_amount = attributes[:'net_asset_value_amount']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          net_asset_value_amount == o.net_asset_value_amount &&
    ...
    def hash
      [net_asset_value_amount, fund_code, fund_name, investment_amount, currency_code, maturity_date, available_units, settled_units, assigned_units, confirmed_units, pending_units, block_units, unrealized_gains, unrealized_percentage_gain, latest_price].hash
    end
