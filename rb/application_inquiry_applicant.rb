...
module SwaggerClient
  class ApplicationInquiryApplicant
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ApplicationInquiryApplicant` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ApplicationInquiryApplicant`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [mother_maiden_name, name, demographics, address, email, phone, contact_preference, contact_consent, financial_information, education, employment_details, identification_document_details, additional_data, partner_customer_details, consent_details, marketing_consent, self_declaration, kyc_information].hash
    end
