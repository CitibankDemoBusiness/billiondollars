  ...
  class ReadyCreditProduct
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ReadyCreditProduct` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ReadyCreditProduct`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [product_code, source_code, organization, logo, request_credit_shield, emboss_name, billing_address, card_delivery_address, pin_delivery_address, gift_code, credit_limit_increase_indicator, atm_required_flag, cheque_book_required_flag, balance_transfer_details, loan_purpose, requested_credit_limit, equal_payment_plan, atm_pin_registration_flag, credit_limit_accepted_flag].hash
    end
