...
module SwaggerClient
  class UnsecuredLoanProductUpdate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::UnsecuredLoanProductUpdate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::UnsecuredLoanProductUpdate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [product_code, source_code, organization, logo, request_credit_shield, billing_address, gift_code, requested_loan_amount, disbursement_option, loan_purpose, tenor, tenor1, penalty_scheme, loan_repayment_details, loan_payment_account_details].hash
    end
