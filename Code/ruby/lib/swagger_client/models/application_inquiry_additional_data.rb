...
module SwaggerClient
  class ApplicationInquiryAdditionalData
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ApplicationInquiryAdditionalData` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ApplicationInquiryAdditionalData`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [number_of_dependents, staff_indicator, business_nature, emergency_contact_name, emergency_contact_phone_number, over_limit_consent_flag, country_specific_group, referral_given_name, referral_surname, related_to_citi_employee_flag, related_citi_employee_name, related_citi_employee_department].hash
    end
