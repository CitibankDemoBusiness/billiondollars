/* 
 * Money Movement
 *
 * <P>The Money Movement API allow you to move money from Citi customers account. Transfers can be made to predefined payees by customer and individuals not included in the payee list</P> The segment of API will allow to perform below category of transfers:<br><br><b>Personal Transfer-</b> Allowing customer to initiate the money transfers within customer�s own Citi accounts.<br><br><b>Internal transfer-</b> Allowing customer to initiate the money transfer from customer Citi own accounts to other Citi accounts locally.<br><br><b>External transfer-</b> Allowing customer to initiate the money transfer from customer Citi own accounts to other bank accounts locally.<br><br><b>Bill Payment-</b> Allowing customer to initiate the bill payment from customer Citi own accounts to billing organization.<br><br><b>Citi Global Transfer-</b> Allowing customer to initiate the money transfer from customer Citi own accounts to other Citi bank accounts across various Countries.<br><br><b>SEPA Transfer-</b> Allowing customer to initiate money transfer to other European bank accounts within the SEPA territory.<br><br><b>Cross border transfer-</b> Allowing customer to initiate money transfer from customer own account to cross border bank accounts.<br>
 *
 * OpenAPI spec version: 1.0.0e
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */
using System;
using System.Linq;
using System.IO;
using System.Text;
using System.Text.RegularExpressions;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Runtime.Serialization;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using System.ComponentModel.DataAnnotations;
using SwaggerDateConverter = IO.Swagger.Client.SwaggerDateConverter;

namespace IO.Swagger.Model
{
    /// <summary>
    /// CrossBorderWireTransferPayee
    /// </summary>
    [DataContract]
        public partial class CrossBorderWireTransferPayee :  IEquatable<CrossBorderWireTransferPayee>, IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="CrossBorderWireTransferPayee" /> class.
        /// </summary>
        /// <param name="payeeName">Name of the payee. (required).</param>
        /// <param name="payeeNickName">The nick name of the payee assigned by the customer. (required).</param>
        /// <param name="payeeType">Identifies the type of payee based on the destination of the payee.This is a reference data field. Please use /v1/utilities/referenceData/{payeeType} resource to get valid value of this field with description.  (required).</param>
        /// <param name="payeeStatus">The payee status. This is a reference data field. Please use /v1/utilities/referenceData/{payeeStatus} resource to get valid value of this field with description..</param>
        /// <param name="transferPurpose">This is to indicate the purpose of transfer. This is a reference data field.This is a reference data field. Please use /v1/utilities/referenceData/{transferPurpose} resource to get valid value of this field with description..</param>
        /// <param name="currencyCode">The currency code of the payee account in ISO 4217 format.</param>
        /// <param name="localCurrencyCode">This is the local currency code in ISO 4217 Format..</param>
        /// <param name="payeeCountryCode">ISO country code. This is a reference data field. Please use /v1/utilities/referenceData/{country} resource to get valid value of this field with description. (required).</param>
        /// <param name="accountId">The payee account identifier in encrypted format. (required).</param>
        /// <param name="displayAccountNumber">A masked account number that can be displayed to the customer. (required).</param>
        /// <param name="beneficiaryBankName">Name of the bank. (required).</param>
        /// <param name="beneficiaryBankCode">The bank code of the external payee account. (required).</param>
        /// <param name="bankRoutingMethod">Bank rounting method.This is a reference data field. Please use /utilities/referenceData/{bankRoutingMethod} resource to get valid values of this field with descriptions.</param>
        /// <param name="payeeAddress">payeeAddress.</param>
        /// <param name="bankAddress">bankAddress.</param>
        /// <param name="crossBorderBeneficiaryDetails">crossBorderBeneficiaryDetails.</param>
        /// <param name="payeeNotes">Free text that usually describes purpose of payment. (required).</param>
        public CrossBorderWireTransferPayee(string payeeName = default(string), string payeeNickName = default(string), string payeeType = default(string), string payeeStatus = default(string), string transferPurpose = default(string), string currencyCode = default(string), string localCurrencyCode = default(string), string payeeCountryCode = default(string), string accountId = default(string), string displayAccountNumber = default(string), string beneficiaryBankName = default(string), string beneficiaryBankCode = default(string), string bankRoutingMethod = default(string), PayeeAddress payeeAddress = default(PayeeAddress), BankAddress bankAddress = default(BankAddress), CrossBorderBeneficiaryDetails crossBorderBeneficiaryDetails = default(CrossBorderBeneficiaryDetails), string payeeNotes = default(string))
        {
            // to ensure "payeeName" is required (not null)
            if (payeeName == null)
            {
                throw new InvalidDataException("payeeName is a required property for CrossBorderWireTransferPayee and cannot be null");
            }
            else
            {
                this.PayeeName = payeeName;
            }
            // to ensure "payeeNickName" is required (not null)
            if (payeeNickName == null)
            {
                throw new InvalidDataException("payeeNickName is a required property for CrossBorderWireTransferPayee and cannot be null");
            }
            else
            {
                this.PayeeNickName = payeeNickName;
            }
            // to ensure "payeeType" is required (not null)
            if (payeeType == null)
            {
                throw new InvalidDataException("payeeType is a required property for CrossBorderWireTransferPayee and cannot be null");
            }
            else
            {
                this.PayeeType = payeeType;
            }
            // to ensure "payeeCountryCode" is required (not null)
            if (payeeCountryCode == null)
            {
                throw new InvalidDataException("payeeCountryCode is a required property for CrossBorderWireTransferPayee and cannot be null");
            }
            else
            {
                this.PayeeCountryCode = payeeCountryCode;
            }
            // to ensure "accountId" is required (not null)
            if (accountId == null)
            {
                throw new InvalidDataException("accountId is a required property for CrossBorderWireTransferPayee and cannot be null");
            }
            else
            {
                this.AccountId = accountId;
            }
            // to ensure "displayAccountNumber" is required (not null)
            if (displayAccountNumber == null)
            {
                throw new InvalidDataException("displayAccountNumber is a required property for CrossBorderWireTransferPayee and cannot be null");
            }
            else
            {
                this.DisplayAccountNumber = displayAccountNumber;
            }
            // to ensure "beneficiaryBankName" is required (not null)
            if (beneficiaryBankName == null)
            {
                throw new InvalidDataException("beneficiaryBankName is a required property for CrossBorderWireTransferPayee and cannot be null");
            }
            else
            {
                this.BeneficiaryBankName = beneficiaryBankName;
            }
            // to ensure "beneficiaryBankCode" is required (not null)
            if (beneficiaryBankCode == null)
            {
                throw new InvalidDataException("beneficiaryBankCode is a required property for CrossBorderWireTransferPayee and cannot be null");
            }
            else
            {
                this.BeneficiaryBankCode = beneficiaryBankCode;
            }
            // to ensure "payeeNotes" is required (not null)
            if (payeeNotes == null)
            {
                throw new InvalidDataException("payeeNotes is a required property for CrossBorderWireTransferPayee and cannot be null");
            }
            else
            {
                this.PayeeNotes = payeeNotes;
            }
            this.PayeeStatus = payeeStatus;
            this.TransferPurpose = transferPurpose;
            this.CurrencyCode = currencyCode;
            this.LocalCurrencyCode = localCurrencyCode;
            this.BankRoutingMethod = bankRoutingMethod;
            this.PayeeAddress = payeeAddress;
            this.BankAddress = bankAddress;
            this.CrossBorderBeneficiaryDetails = crossBorderBeneficiaryDetails;
        }
        
        /// <summary>
        /// Name of the payee.
        /// </summary>
        /// <value>Name of the payee.</value>
        [DataMember(Name="payeeName", EmitDefaultValue=false)]
        public string PayeeName { get; set; }

        /// <summary>
        /// The nick name of the payee assigned by the customer.
        /// </summary>
        /// <value>The nick name of the payee assigned by the customer.</value>
        [DataMember(Name="payeeNickName", EmitDefaultValue=false)]
        public string PayeeNickName { get; set; }

        /// <summary>
        /// Identifies the type of payee based on the destination of the payee.This is a reference data field. Please use /v1/utilities/referenceData/{payeeType} resource to get valid value of this field with description. 
        /// </summary>
        /// <value>Identifies the type of payee based on the destination of the payee.This is a reference data field. Please use /v1/utilities/referenceData/{payeeType} resource to get valid value of this field with description. </value>
        [DataMember(Name="payeeType", EmitDefaultValue=false)]
        public string PayeeType { get; set; }

        /// <summary>
        /// The payee status. This is a reference data field. Please use /v1/utilities/referenceData/{payeeStatus} resource to get valid value of this field with description.
        /// </summary>
        /// <value>The payee status. This is a reference data field. Please use /v1/utilities/referenceData/{payeeStatus} resource to get valid value of this field with description.</value>
        [DataMember(Name="payeeStatus", EmitDefaultValue=false)]
        public string PayeeStatus { get; set; }

        /// <summary>
        /// This is to indicate the purpose of transfer. This is a reference data field.This is a reference data field. Please use /v1/utilities/referenceData/{transferPurpose} resource to get valid value of this field with description.
        /// </summary>
        /// <value>This is to indicate the purpose of transfer. This is a reference data field.This is a reference data field. Please use /v1/utilities/referenceData/{transferPurpose} resource to get valid value of this field with description.</value>
        [DataMember(Name="transferPurpose", EmitDefaultValue=false)]
        public string TransferPurpose { get; set; }

        /// <summary>
        /// The currency code of the payee account in ISO 4217 format
        /// </summary>
        /// <value>The currency code of the payee account in ISO 4217 format</value>
        [DataMember(Name="currencyCode", EmitDefaultValue=false)]
        public string CurrencyCode { get; set; }

        /// <summary>
        /// This is the local currency code in ISO 4217 Format.
        /// </summary>
        /// <value>This is the local currency code in ISO 4217 Format.</value>
        [DataMember(Name="localCurrencyCode", EmitDefaultValue=false)]
        public string LocalCurrencyCode { get; set; }

        /// <summary>
        /// ISO country code. This is a reference data field. Please use /v1/utilities/referenceData/{country} resource to get valid value of this field with description.
        /// </summary>
        /// <value>ISO country code. This is a reference data field. Please use /v1/utilities/referenceData/{country} resource to get valid value of this field with description.</value>
        [DataMember(Name="payeeCountryCode", EmitDefaultValue=false)]
        public string PayeeCountryCode { get; set; }

        /// <summary>
        /// The payee account identifier in encrypted format.
        /// </summary>
        /// <value>The payee account identifier in encrypted format.</value>
        [DataMember(Name="accountId", EmitDefaultValue=false)]
        public string AccountId { get; set; }

        /// <summary>
        /// A masked account number that can be displayed to the customer.
        /// </summary>
        /// <value>A masked account number that can be displayed to the customer.</value>
        [DataMember(Name="displayAccountNumber", EmitDefaultValue=false)]
        public string DisplayAccountNumber { get; set; }

        /// <summary>
        /// Name of the bank.
        /// </summary>
        /// <value>Name of the bank.</value>
        [DataMember(Name="beneficiaryBankName", EmitDefaultValue=false)]
        public string BeneficiaryBankName { get; set; }

        /// <summary>
        /// The bank code of the external payee account.
        /// </summary>
        /// <value>The bank code of the external payee account.</value>
        [DataMember(Name="beneficiaryBankCode", EmitDefaultValue=false)]
        public string BeneficiaryBankCode { get; set; }

        /// <summary>
        /// Bank rounting method.This is a reference data field. Please use /utilities/referenceData/{bankRoutingMethod} resource to get valid values of this field with descriptions
        /// </summary>
        /// <value>Bank rounting method.This is a reference data field. Please use /utilities/referenceData/{bankRoutingMethod} resource to get valid values of this field with descriptions</value>
        [DataMember(Name="bankRoutingMethod", EmitDefaultValue=false)]
        public string BankRoutingMethod { get; set; }

        /// <summary>
        /// Gets or Sets PayeeAddress
        /// </summary>
        [DataMember(Name="payeeAddress", EmitDefaultValue=false)]
        public PayeeAddress PayeeAddress { get; set; }

        /// <summary>
        /// Gets or Sets BankAddress
        /// </summary>
        [DataMember(Name="bankAddress", EmitDefaultValue=false)]
        public BankAddress BankAddress { get; set; }

        /// <summary>
        /// Gets or Sets CrossBorderBeneficiaryDetails
        /// </summary>
        [DataMember(Name="crossBorderBeneficiaryDetails", EmitDefaultValue=false)]
        public CrossBorderBeneficiaryDetails CrossBorderBeneficiaryDetails { get; set; }

        /// <summary>
        /// Free text that usually describes purpose of payment.
        /// </summary>
        /// <value>Free text that usually describes purpose of payment.</value>
        [DataMember(Name="payeeNotes", EmitDefaultValue=false)]
        public string PayeeNotes { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class CrossBorderWireTransferPayee {\n");
            sb.Append("  PayeeName: ").Append(PayeeName).Append("\n");
            sb.Append("  PayeeNickName: ").Append(PayeeNickName).Append("\n");
            sb.Append("  PayeeType: ").Append(PayeeType).Append("\n");
            sb.Append("  PayeeStatus: ").Append(PayeeStatus).Append("\n");
            sb.Append("  TransferPurpose: ").Append(TransferPurpose).Append("\n");
            sb.Append("  CurrencyCode: ").Append(CurrencyCode).Append("\n");
            sb.Append("  LocalCurrencyCode: ").Append(LocalCurrencyCode).Append("\n");
            sb.Append("  PayeeCountryCode: ").Append(PayeeCountryCode).Append("\n");
            sb.Append("  AccountId: ").Append(AccountId).Append("\n");
            sb.Append("  DisplayAccountNumber: ").Append(DisplayAccountNumber).Append("\n");
            sb.Append("  BeneficiaryBankName: ").Append(BeneficiaryBankName).Append("\n");
            sb.Append("  BeneficiaryBankCode: ").Append(BeneficiaryBankCode).Append("\n");
            sb.Append("  BankRoutingMethod: ").Append(BankRoutingMethod).Append("\n");
            sb.Append("  PayeeAddress: ").Append(PayeeAddress).Append("\n");
            sb.Append("  BankAddress: ").Append(BankAddress).Append("\n");
            sb.Append("  CrossBorderBeneficiaryDetails: ").Append(CrossBorderBeneficiaryDetails).Append("\n");
            sb.Append("  PayeeNotes: ").Append(PayeeNotes).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }
  
        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="input">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object input)
        {
            return this.Equals(input as CrossBorderWireTransferPayee);
        }

        /// <summary>
        /// Returns true if CrossBorderWireTransferPayee instances are equal
        /// </summary>
        /// <param name="input">Instance of CrossBorderWireTransferPayee to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(CrossBorderWireTransferPayee input)
        {
            if (input == null)
                return false;

            return 
                (
                    this.PayeeName == input.PayeeName ||
                    (this.PayeeName != null &&
                    this.PayeeName.Equals(input.PayeeName))
                ) && 
                (
                    this.PayeeNickName == input.PayeeNickName ||
                    (this.PayeeNickName != null &&
                    this.PayeeNickName.Equals(input.PayeeNickName))
                ) && 
                (
                    this.PayeeType == input.PayeeType ||
                    (this.PayeeType != null &&
                    this.PayeeType.Equals(input.PayeeType))
                ) && 
                (
                    this.PayeeStatus == input.PayeeStatus ||
                    (this.PayeeStatus != null &&
                    this.PayeeStatus.Equals(input.PayeeStatus))
                ) && 
                (
                    this.TransferPurpose == input.TransferPurpose ||
                    (this.TransferPurpose != null &&
                    this.TransferPurpose.Equals(input.TransferPurpose))
                ) && 
                (
                    this.CurrencyCode == input.CurrencyCode ||
                    (this.CurrencyCode != null &&
                    this.CurrencyCode.Equals(input.CurrencyCode))
                ) && 
                (
                    this.LocalCurrencyCode == input.LocalCurrencyCode ||
                    (this.LocalCurrencyCode != null &&
                    this.LocalCurrencyCode.Equals(input.LocalCurrencyCode))
                ) && 
                (
                    this.PayeeCountryCode == input.PayeeCountryCode ||
                    (this.PayeeCountryCode != null &&
                    this.PayeeCountryCode.Equals(input.PayeeCountryCode))
                ) && 
                (
                    this.AccountId == input.AccountId ||
                    (this.AccountId != null &&
                    this.AccountId.Equals(input.AccountId))
                ) && 
                (
                    this.DisplayAccountNumber == input.DisplayAccountNumber ||
                    (this.DisplayAccountNumber != null &&
                    this.DisplayAccountNumber.Equals(input.DisplayAccountNumber))
                ) && 
                (
                    this.BeneficiaryBankName == input.BeneficiaryBankName ||
                    (this.BeneficiaryBankName != null &&
                    this.BeneficiaryBankName.Equals(input.BeneficiaryBankName))
                ) && 
                (
                    this.BeneficiaryBankCode == input.BeneficiaryBankCode ||
                    (this.BeneficiaryBankCode != null &&
                    this.BeneficiaryBankCode.Equals(input.BeneficiaryBankCode))
                ) && 
                (
                    this.BankRoutingMethod == input.BankRoutingMethod ||
                    (this.BankRoutingMethod != null &&
                    this.BankRoutingMethod.Equals(input.BankRoutingMethod))
                ) && 
                (
                    this.PayeeAddress == input.PayeeAddress ||
                    (this.PayeeAddress != null &&
                    this.PayeeAddress.Equals(input.PayeeAddress))
                ) && 
                (
                    this.BankAddress == input.BankAddress ||
                    (this.BankAddress != null &&
                    this.BankAddress.Equals(input.BankAddress))
                ) && 
                (
                    this.CrossBorderBeneficiaryDetails == input.CrossBorderBeneficiaryDetails ||
                    (this.CrossBorderBeneficiaryDetails != null &&
                    this.CrossBorderBeneficiaryDetails.Equals(input.CrossBorderBeneficiaryDetails))
                ) && 
                (
                    this.PayeeNotes == input.PayeeNotes ||
                    (this.PayeeNotes != null &&
                    this.PayeeNotes.Equals(input.PayeeNotes))
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                int hashCode = 41;
                if (this.PayeeName != null)
                    hashCode = hashCode * 59 + this.PayeeName.GetHashCode();
                if (this.PayeeNickName != null)
                    hashCode = hashCode * 59 + this.PayeeNickName.GetHashCode();
                if (this.PayeeType != null)
                    hashCode = hashCode * 59 + this.PayeeType.GetHashCode();
                if (this.PayeeStatus != null)
                    hashCode = hashCode * 59 + this.PayeeStatus.GetHashCode();
                if (this.TransferPurpose != null)
                    hashCode = hashCode * 59 + this.TransferPurpose.GetHashCode();
                if (this.CurrencyCode != null)
                    hashCode = hashCode * 59 + this.CurrencyCode.GetHashCode();
                if (this.LocalCurrencyCode != null)
                    hashCode = hashCode * 59 + this.LocalCurrencyCode.GetHashCode();
                if (this.PayeeCountryCode != null)
                    hashCode = hashCode * 59 + this.PayeeCountryCode.GetHashCode();
                if (this.AccountId != null)
                    hashCode = hashCode * 59 + this.AccountId.GetHashCode();
                if (this.DisplayAccountNumber != null)
                    hashCode = hashCode * 59 + this.DisplayAccountNumber.GetHashCode();
                if (this.BeneficiaryBankName != null)
                    hashCode = hashCode * 59 + this.BeneficiaryBankName.GetHashCode();
                if (this.BeneficiaryBankCode != null)
                    hashCode = hashCode * 59 + this.BeneficiaryBankCode.GetHashCode();
                if (this.BankRoutingMethod != null)
                    hashCode = hashCode * 59 + this.BankRoutingMethod.GetHashCode();
                if (this.PayeeAddress != null)
                    hashCode = hashCode * 59 + this.PayeeAddress.GetHashCode();
                if (this.BankAddress != null)
                    hashCode = hashCode * 59 + this.BankAddress.GetHashCode();
                if (this.CrossBorderBeneficiaryDetails != null)
                    hashCode = hashCode * 59 + this.CrossBorderBeneficiaryDetails.GetHashCode();
                if (this.PayeeNotes != null)
                    hashCode = hashCode * 59 + this.PayeeNotes.GetHashCode();
                return hashCode;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }
}
