  ...
  class CreditCardAccount
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::CreditCardAccount` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::CreditCardAccount`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'credit_used')
        self.credit_used = attributes[:'credit_used']
      end
    ...
    def hash
      [product_name, product_code, display_account_number, currency_code, outstanding_balance, last_statement_balance, last_statement_date, credit_used, credit_limit, available_credit, cash_advance_limit, cash_advance_available_amount, last_payment_amount, last_payment_date, minimum_due_amount, payment_due_date, available_point_balance].hash
    end
