...
module SwaggerClient
  class LoanBookingRequest
    ...
    attr_accessor :payment_reference_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::LoanBookingRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::LoanBookingRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'payment_reference_id')
        self.payment_reference_id = attributes[:'payment_reference_id']
      end
    end
    ...
    def hash
      [payee_id, loan_amount, disbursement_option, disbursement_account_id, tenor, bank_details, loan_purpose, remarks, tax_id, payment_reference_id].hash
    end
