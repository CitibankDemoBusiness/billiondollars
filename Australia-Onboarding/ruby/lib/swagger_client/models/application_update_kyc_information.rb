...
module SwaggerClient
  class ApplicationUpdateKycInformation
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ApplicationUpdateKycInformation` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ApplicationUpdateKycInformation`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [self_public_figure_declaration_flag, public_figure_office_status, public_figure_office_details, public_figure_office_start_date, public_figure_office_end_date, is_related_to_senior_public_figure, related_senior_public_figure_name, related_spf_country_of_government, related_senior_public_figure_department, relationship_with_senior_public_figure, related_senior_public_figure_last_name, us_tax_status, us_tax_id].hash
    end
